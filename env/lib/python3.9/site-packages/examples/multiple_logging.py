...
def process_files():
    ...
    for num in range(SPLINES):  # pylint: disable=unused-variable
        time.sleep(secrets.SystemRandom().uniform(0.1, 0.5))  # Random processing time
        prep.update()
    prep.close()
    ...
    for dnum in range(1, DATACENTERS + 1):
        systems = secrets.SystemRandom().randint(*SYSTEMS)  # Random number of systems
        ...
        for snum in range(1, systems + 1):
            ...
            files = secrets.SystemRandom().randint(*FILES)  # Random file count

            # Iterate through files
            for fnum in range(files):  # pylint: disable=unused-variable
                system.update()  # Update count
                time.sleep(secrets.SystemRandom().uniform(0.0001, 0.0005))  # Random processing time
